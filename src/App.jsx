...
import 'swiper/css';
import { 
  Play, Clock, MagnifyingGlass, CirclesThreePlus, CaretDown,
  Faders, Bell, InstagramLogo, YoutubeLogo, LinkedinLogo, User
} from '@phosphor-icons/react';

function App() {
  // --- URL BASE PARA ASSETS (IMPORTANTE PARA O ROBÔ) ---
  const baseUrl = import.meta.env.BASE_URL;
  
  const [activeModuleId, setActiveModuleId] = useState(modulesData[0]?.id || 1);
  const [lastSession, setLastSession] = useState({ 
    moduleId: 1, submoduleIndex: 0, title: modulesData[0]?.submodules?.[0]?.title || "Introdução"
  });
    ...
    <div className="min-h-screen flex flex-col relative bg-[#020617] text-slate-50 font-sans">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2 cursor-pointer group">
                <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center font-bold italic group-hover:scale-110 transition text-white">E</div>
                <span className="font-bold text-xl hidden md:block tracking-tight text-slate-200">Ensino<span className="text-blue-500">Med</span></span>
            </div>
            <nav className="hidden lg:flex gap-6 text-sm font-medium">
              {navLinksList.map((link) => (
                <a key={link} href="#" onClick={(e) => { e.preventDefault(); setActiveNavLink(link); }} className={`transition-colors ${activeNavLink === link ? 'text-white font-semibold' : 'text-slate-400 hover:text-white'}`}>{link}</a>
              ))}
            </nav>
          </div>
          <div className="flex-1 flex justify-end md:justify-end gap-4">
             <div className="relative">
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center justify-between gap-2 bg-slate-800/50 border border-slate-700 hover:bg-slate-700 text-white text-sm font-medium px-4 py-2 rounded-xl min-w-[280px] transition-all">
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-lg bg-white flex items-center justify-center"><span className="text-blue-600 text-xs font-bold">MC</span></div>
             ...
             <div className="flex items-center gap-4 border-l border-slate-700 pl-4 relative">
                ...
                <div className="relative">
                    <div onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-9 h-9 bg-gradient-to-br from-purple-500 to-blue-600 rounded-full flex items-center justify-center text-xs font-bold border-2 border-[#1e293b] cursor-pointer hover:scale-105 transition shadow-lg shadow-purple-500/20 text-white select-none">NA</div>
                </div>
             </div>
          </div>
        </div>
      ...
      <div className="w-full bg-[#020617] px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-4 z-40 relative">
        <div className="relative w-full md:w-[480px]">
           <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><MagnifyingGlass size={18} className="text-slate-500" /></div>
           <input type="text" placeholder="Pesquisar Módulos, Aulas, Questões..." className="w-full bg-[#0b1121] border border-slate-800 focus:border-blue-600 text-slate-200 text-sm rounded-full pl-11 pr-4 h-12 transition-all outline-none placeholder:text-slate-600" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
        <div className="flex items-center gap-3">
            <button onClick={handleContinueSession} className="bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold px-6 h-10 rounded-full shadow-lg shadow-blue-900/20 transition-all transform hover:scale-105">Continuar de onde parei</button>
        </div>
      </div>
      ...
      <div className="w-full bg-[#020617] px-6 py-2 flex flex-col md:flex-row items-center justify-center text-xs md:text-sm gap-3 z-30 mb-4">
        <span className="text-slate-500 font-medium tracking-wide">Sua próxima sessão:</span>
        <button onClick={handleContinueSession} className="group flex items-center gap-2 bg-[#0f172a] hover:bg-[#1e293b] border border-slate-800 px-4 py-1.5 rounded-lg transition-all cursor-pointer">
            <span className="text-blue-500 font-bold group-hover:text-blue-400 transition-colors">{modulesData.find(m => m.id === lastSession.moduleId)?.title}: <span className="text-blue-400 font-normal">{lastSession.title}</span></span>
        </button>
      </div>
      ...
      <section className="relative w-full overflow-hidden group border-b border-slate-800 pb-0">
         <div className="absolute inset-0 w-full h-full bg-[#020617]">
            <div className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-all duration-500 transform group-hover:scale-105" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop')", opacity: 0.4 }}></div>
         ...
         <div className="relative z-10 w-full h-full flex items-center px-6 lg:px-12 pt-12 pb-6">
            <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-center h-full">
                <div className="flex flex-col gap-6 w-full lg:w-3/4 transition-all duration-500 items-center text-center">
                    <div>
                        <h1 className="text-5xl md:text-7xl font-bold text-white tracking-tight drop-shadow-2xl">{activeModuleData?.title || "Carregando..."}</h1>
                        <p className="text-slate-400 mt-2 text-lg">Fundamentos essenciais para sua jornada.</p>
                    </div>
      ...
      <div className="w-full bg-[#020617] pt-0 pb-12 relative z-30">
        <div className="w-full max-w-7xl mx-auto px-4 mt-10">
            <Swiper onSwiper={(swiper) => (swiperRef.current = swiper)} loop={true} slidesPerView="auto" spaceBetween={40} centeredSlides={true} slideToClickedSlide={true} grabCursor={true} className="mySwiper" onSlideChange={(swiper) => { const newModule = modulesData[swiper.realIndex]; if(newModule) setActiveModuleId(newModule.id); }}>
                {modulesData.map((mod, index) => (
                    <SwiperSlide key={mod.id} className={`!w-[217.6px] ${activeModuleId === mod.id ? 'is-active' : ''}`}>
                        <div className="relative w-[180px] h-full flex flex-col justify-end cursor-pointer group" onClick={() => handleSlideClick(mod, index)}>
                            <div className="relative w-full pb-2">
                                <div className="block-container"><div className="block-content">{mod.title}</div></div>
                                <div className="robot-wrapper">
                                    <img src={`${baseUrl}assets/modules/robo.png`} onError={(e) => e.target.style.display='none'} alt="" className="robot-img" />
                                </div>
                            </div>
            ...
            <div className="max-w-7xl mx-auto px-6" id="submodules-section">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 border-b border-slate-800 pb-6">
                    ...
                    <div className="mt-4 md:mt-0 px-4 py-2 bg-blue-900/30 text-blue-400 rounded-full text-xs font-semibold border border-blue-900/50">
                        <span>{statsSubmodules}</span> Aulas
                    </div>
                </div>
      ...
      <footer className="bg-[#0b1121] border-t border-slate-800 pt-10 pb-10 mt-auto">
        <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                <div className="flex items-center gap-6 text-sm text-slate-400"><a href="#" className="hover:text-white transition-colors">Suporte</a><a href="#" className="hover:text-white transition-colors">Termos</a></div>
                <div className="text-center"><span className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Medicina JVS</span><p className="text-xs text-slate-500 mt-1">Excelência em Preparação Médica</p></div>
                <div className="flex gap-4"><InstagramLogo size={20} className="text-slate-400" weight="fill" /></div>
            </div>
            <div className="border-t border-slate-800/50 mt-8 pt-4 text-center text-xs text-slate-600">&copy; 2025 MedCof. Todos os direitos reservados.</div>
        </div>
      </footer>
    </div>
  );
...
function StatCard({ label, value, isPurple = false }) {
    return (
        <div className={`bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 flex flex-col items-center justify-center transition cursor-pointer hover:bg-white/10 hover:-translate-y-1 group/card`}>
